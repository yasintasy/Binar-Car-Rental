    ...
    const findCarFilter = async () => {
        ...
        try {
            ...
            if (!filterData.carName && !filterData.carCategory && !filterData.carPrice) {
                ...
                const selectResult = newData.filter((car) => car.name?.toLowerCase() === filterData.carName.toLowerCase());
                setCars(selectResult)
            } else if (filterData.carName && filterData.carCategory && !filterData.carPrice) {
                const selectResult = newData.filter((car) => car.name?.toLowerCase() === filterData.carName.toLowerCase() && car.category?.toLowerCase() === filterData.carCategory.toLowerCase());
                setCars(selectResult)
            } else {
                if (filterData.carPrice === '400000') {
                    console.log("a")
                    const selectResult = newData.filter((car) => car.name?.toLowerCase() === filterData.carName.toLowerCase() && car.category?.toLowerCase() === filterData.carCategory.toLowerCase() && filterData.carPrice >= car.price);
                    setCars(selectResult)
                } else if (filterData.carPrice === '400000-600000') {
                    const selectResult = newData.filter((car) => car.name?.toLowerCase() === filterData.carName.toLowerCase() && car.category?.toLowerCase() === filterData.carCategory.toLowerCase() && (car.price >= 400000 && car.price <= 600000));
                    setCars(selectResult)
                } else {
                    const selectResult = newData.filter((car) => car.name?.toLowerCase() === filterData.carName.toLowerCase() && car.category?.toLowerCase() === filterData.carCategory.toLowerCase() && car.price >= 600000);
                    setCars(selectResult)
                }
            }
        } catch (error) {
